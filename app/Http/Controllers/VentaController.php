...
class VentaController extends Controller
{
     ...
     public function index(Request $request)
    {
        if ($request)
        {
            ...
            ->select('v.idventa','v.fechahora','p.nombre','v.tipo_comprobante','v.num_comprobante','v.impuesto','v.estado','v.total_venta')

            ->where('v.num_comprobante','LIKE','%'.$query.'%')
            ->orderBy('v.idventa','desc')
            ->groupBy('v.idventa','v.fechahora','p.nombre','v.tipo_comprobante','v.num_comprobante','v.impuesto','v.estado')
            ->paginate(7);
